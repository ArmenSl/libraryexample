...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {"name": "Publisher", "description": "Operations for Publisher entities"},
        {"name": "Publisher Relationships", "description": "Manage Publisher relationships"},
...
def get_statistics(database: Session = Depends(get_db)):
    ...
    stats = {}
    stats["publisher_count"] = database.query(Publisher).count()
...
@app.get("/publisher/", response_model=None, tags=["Publisher"])
def get_all_publisher(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    from sqlalchemy.orm import joinedload

    # Use detailed=true to get entities with eagerly loaded relationships (for tables with lookup columns)
    if detailed:
        # Eagerly load all relationships to avoid N+1 queries
        query = database.query(Publisher)
        publisher_list = query.all()

        # Serialize with relationships included
        result = []
        for publisher_item in publisher_list:
            item_dict = publisher_item.__dict__.copy()
            item_dict.pop('_sa_instance_state', None)

            # Add many-to-one relationships (foreign keys for lookup columns)

            # Add many-to-many and one-to-many relationship objects (full details)
            book_list = database.query(Book).filter(Book.publisher_id == publisher_item.id).all()
            item_dict['book'] = []
        ...
        return database.query(Publisher).all()


@app.get("/publisher/count/", response_model=None, tags=["Publisher"])
def get_count_publisher(database: Session = Depends(get_db)) -> dict:
    """Get the total count of Publisher entities"""
    count = database.query(Publisher).count()
    return {"count": count}


@app.get("/publisher/paginated/", response_model=None, tags=["Publisher"])
def get_paginated_publisher(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    """Get paginated list of Publisher entities"""
    total = database.query(Publisher).count()
    publisher_list = database.query(Publisher).offset(skip).limit(limit).all()
    ...
    result = []
    for publisher_item in publisher_list:
        book_ids = database.query(Book.id).filter(Book.publisher_id == publisher_item.id).all()
        item_data = {
...
@app.get("/publisher/search/", response_model=None, tags=["Publisher"])
def search_publisher(
    database: Session = Depends(get_db)
) -> list:
    """Search Publisher entities by attributes"""
    query = database.query(Publisher)
...
@app.get("/publisher/{publisher_id}/", response_model=None, tags=["Publisher"])
async def get_publisher(publisher_id: int, database: Session = Depends(get_db)) -> Publisher:
    db_publisher = database.query(Publisher).filter(Publisher.id == publisher_id).first()
    if db_publisher is None:
        raise HTTPException(status_code=404, detail="Publisher not found")

    book_ids = database.query(Book.id).filter(Book.publisher_id == db_publisher.id).all()
    response_data = {
...
@app.post("/publisher/", response_model=None, tags=["Publisher"])
async def create_publisher(publisher_data: PublisherCreate, database: Session = Depends(get_db)) -> Publisher:


    db_publisher = Publisher(
        telephone=publisher_data.telephone,        address=publisher_data.address,        name=publisher_data.name        )

    database.add(db_publisher)
    database.commit()
    database.refresh(db_publisher)

    if publisher_data.book:
        # Validate that all Book IDs exist
        for book_id in publisher_data.book:
        ...
        database.query(Book).filter(Book.id.in_(publisher_data.book)).update(
            {Book.publisher_id: db_publisher.id}, synchronize_session=False
        )
        database.commit()
...
@app.post("/publisher/bulk/", response_model=None, tags=["Publisher"])
async def bulk_create_publisher(items: list[PublisherCreate], database: Session = Depends(get_db)) -> dict:
    """Create multiple Publisher entities at once"""
    created_items = []
    errors = []
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_publisher = Publisher(
                telephone=item_data.telephone,                address=item_data.address,                name=item_data.name            )
            database.add(db_publisher)
            database.flush()  # Get ID without committing
            created_items.append(db_publisher.id)
    ...
    return {
        ...
        "message": f"Successfully created {len(created_items)} Publisher entities"
    }


@app.delete("/publisher/bulk/", response_model=None, tags=["Publisher"])
async def bulk_delete_publisher(ids: list[int], database: Session = Depends(get_db)) -> dict:
    """Delete multiple Publisher entities at once"""
    deleted_count = 0
    not_found = []

    for item_id in ids:
        db_publisher = database.query(Publisher).filter(Publisher.id == item_id).first()
        if db_publisher:
            database.delete(db_publisher)
            deleted_count += 1
        else:
    ...
    return {
        ...
        "not_found": not_found,
        "message": f"Successfully deleted {deleted_count} Publisher entities"
    }

@app.put("/publisher/{publisher_id}/", response_model=None, tags=["Publisher"])
async def update_publisher(publisher_id: int, publisher_data: PublisherCreate, database: Session = Depends(get_db)) -> Publisher:
    db_publisher = database.query(Publisher).filter(Publisher.id == publisher_id).first()
    if db_publisher is None:
        raise HTTPException(status_code=404, detail="Publisher not found")

    setattr(db_publisher, 'telephone', publisher_data.telephone)
    setattr(db_publisher, 'address', publisher_data.address)
    setattr(db_publisher, 'name', publisher_data.name)
    if publisher_data.book is not None:
        # Clear all existing relationships (set foreign key to NULL)
        database.query(Book).filter(Book.publisher_id == db_publisher.id).update(
            {Book.publisher_id: None}, synchronize_session=False
        )
...
@app.delete("/publisher/{publisher_id}/", response_model=None, tags=["Publisher"])
async def delete_publisher(publisher_id: int, database: Session = Depends(get_db)):
    db_publisher = database.query(Publisher).filter(Publisher.id == publisher_id).first()
    if db_publisher is None:
        raise HTTPException(status_code=404, detail="Publisher not found")
    database.delete(db_publisher)
...
@app.get("/publisher/{publisher_id}/book/", response_model=None, tags=["Publisher Relationships"])
async def get_book_of_publisher(publisher_id: int, database: Session = Depends(get_db)):
    """Get all Book entities related to this Publisher through book"""
    db_publisher = database.query(Publisher).filter(Publisher.id == publisher_id).first()
    if db_publisher is None:
        raise HTTPException(status_code=404, detail="Publisher not found")

    book_list = database.query(Book).filter(Book.publisher_id == publisher_id).all()
...
async def create_library(library_data: LibraryCreate, database: Session = Depends(get_db)) -> Library:
    ...
    db_library = Library(
        telephone=library_data.telephone,        address=library_data.address,        web_page=library_data.web_page,        name=library_data.name        )
...
async def bulk_create_library(items: list[LibraryCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_library = Library(
                telephone=item_data.telephone,                address=item_data.address,                web_page=item_data.web_page,                name=item_data.name            )
            database.add(db_library)
            database.flush()  # Get ID without committing
...
async def execute_library_cheapest_book_by(
    ...
    try:
        ...
        async def wrapper(_library_object):
            cheapest: Book = None
            price = 1000000000.0
            book_8749227624211 = (await get_books_of_library(_library_object.id, database))['books']
            for i in range(0, len(book_8749227624211)):
                book = book_8749227624211[i]
...
def get_all_book(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        query = query.options(joinedload(Book.publisher))
        book_list = query.all()
        ...
        for book_item in book_list:
            ...
            if book_item.publisher:
                related_obj = book_item.publisher
                related_dict = related_obj.__dict__.copy()
                related_dict.pop('_sa_instance_state', None)
                item_dict['publisher'] = related_dict
            else:
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    if book_data.publisher is not None:
        db_publisher = database.query(Publisher).filter(Publisher.id == book_data.publisher).first()
        if not db_publisher:
            raise HTTPException(status_code=400, detail="Publisher not found")
    else:
        raise HTTPException(status_code=400, detail="Publisher ID is required")
    ...
    db_book = Book(
        pages=book_data.pages,        genre=book_data.genre.value,        price=book_data.price,        release=book_data.release,        title=book_data.title,        stock=book_data.stock,        publisher_id=book_data.publisher        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            # Basic validation for each item
            if not item_data.publisher:
                raise ValueError("Publisher ID is required")

            db_book = Book(
                pages=item_data.pages,                genre=item_data.genre.value,                price=item_data.price,                release=item_data.release,                title=item_data.title,                stock=item_data.stock,                publisher_id=item_data.publisher            )
            database.add(db_book)
            database.flush()  # Get ID without committing
...
async def update_book(book_id: int, book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    if book_data.publisher is not None:
        ...
        if not db_publisher:
            raise HTTPException(status_code=400, detail="Publisher not found")
        setattr(db_book, 'publisher_id', book_data.publisher)
